...
export function RegisterForm({ onSuccess, onSwitchToLogin }: RegisterFormProps) {
  ...
  const handleOAuthSignup = async (provider: 'google' | 'github') => {
    ...
    try {
      ...
      if (data.auth_url) {
        // Store state in cookie (persists across domain redirects)
        document.cookie = `oauth_state=${data.state}; path=/; max-age=600; secure; samesite=lax`;
        document.cookie = `oauth_provider=${provider}; path=/; max-age=600; secure; samesite=lax`;
        window.location.href = data.auth_url;
      } else {
        toast.error('Failed to initiate OAuth signup');
        setIsLoading(false);
      }
    } catch (error) {
